...
namespace PeterO
{
    ...
    /// <param name='x'>A string object.</param>
      ...
      new Regex("^hsla\\(\\s*([\\+\\-]?\\d+(?:\\.\\d+)?)\\s*,\\s*([\\+\\-]?\\d+(?:\\.\\d+)?)%\\s*,\\s*([\\+\\-]?\\d+(?:\\.\\d+)?)%\\s*,\\s*([\\+\\-]?\\d+(?:\\.\\d+)?)\\s*\\)$")
        ...
        private static int GetHex(string str)
        {
            ...
            if (str.Length == 0) throw new ArgumentException("s is empty");
    ...
    /// <returns>A string object.</returns>
        public static string RgbToColor(double[] arrayRGB)
        {
            ...
            {
                double prec = Math.Round((arrayRGB[3] / 255.0) * 100.0, MidpointRounding.AwayFromZero) / 100.0;
    ...
    /// <param name='rgb'>A double[] object.</param>
        public static string RgbToColorHtml(double[] rgb)
        {
            ...
            c = ((c = (int)Math.Round(rgb[1], MidpointRounding.AwayFromZero)) < 0 ? 0 : (c > 255 ? 255 : c));
            sb.Append(HexArray[(c >> 4) & 15]);
            sb.Append(HexArray[(c) & 15]);
            c = ((c = (int)Math.Round(rgb[2], MidpointRounding.AwayFromZero)) < 0 ? 0 : (c > 255 ? 255 : c));
            sb.Append(HexArray[(c >> 4) & 15]);
            sb.Append(HexArray[(c) & 15]);
            return sb.ToString();
        }
